...
export const DrawLineText = ({
  ...
  useGSAP(
    () => {
      ...
      tl.to(textChildren, { strokeDashoffset: 0, duration: 2.5, ease: "linear", stagger: oneByOne ? 0.8 : 0 });
      if (afterFill) {
        tl.to(textChildren, {
          fillOpacity: 1,
          duration: 0.6,
  ...
  return (
    ...
    >
      {text.split("").map((char, i) => (
        <text
          key={i}
          style={{ stroke: color, fill: color, fillOpacity: 0, fontSize, strokeWidth: `${strokeWidth}px` }}
          y={fontSize}
        >
          {char}
        </text>
