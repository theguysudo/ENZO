...
(This entry's original body was corrupted — an accidentally-pasted raw
conversation summary had displaced the real notes. Reconstructed from the
session's actual work; the corruption was flagged in the 2026-08-29 entry.)

- **Chat auto-scroll**: the transcript now follows the stream — new tokens
  keep the latest content in view without jumping when the user scrolls up
  (manual scroll pauses the follow; returning to the bottom resumes it).
- **Typing animation**: assistant replies render with a natural typing
  cadence during streaming instead of appearing in bulk chunks.
...
### Note
The `[2026-07-06] - Session Log: Chat Auto-scroll Fix & Typing Animation` entry had contained corrupted content (an accidentally-pasted raw conversation summary, including a fake `<system-reminder>` block). It was replaced on 2026-09-06 with a reconstructed summary of that session's actual work.